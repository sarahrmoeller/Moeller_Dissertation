...
%The problem with manual annotation is not limited to field linguistics; even a project for natural language processing required three years to annotate just one layer of the Penn Treebank \citep{taylor_penn_2003}. 
...
%In addition, unannotated and annotated data cannot be simply combined. Annotated data must be weighted so that the larger, unannotated part does not overwhelm it. 
...
%Second, the glossed information in published IGT snippets can vary widely depending on the phenomenon that is the focus of the publication or the linguist who published.
...
Glossing-only NLP experiments assume that the data is already segmented into morphemes or that it does not need to be segmented. 
...
Work in computational morphology for low-resource languages generally assumes that other interlinear information is available. POS tags are an example of information frequently assumed to be available. For example, 
