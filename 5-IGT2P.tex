...
All models and techniques were tested on the same held-out set chosen randomly from multi-entry paradigms in each language. Results were compared when trained on the noisy paradigms and on the expertly cleaned paradigms. The results are displayed in Table \ref{tab:IGT2Presults}. It shows that the limited involvement of experts cleaning the data always improved results over training on the noisy data. Also the transformer outperformed the LSTM with hard monotonic attention on all instances, The exception to both these statements is Alas [btz]. This perhaps because it is the smallest corpus, roughly half or two-third the size of the next largest corpus (Lezgi). Alas also has the poorest results by far of all the languages.
Comparing results from augmenting the data by artificial and uninflected/unannotated tokens gave varied results. Th factors affecting these varied results are unclear although the consistency of the noisy and the extent of cleaning may be factors. 
