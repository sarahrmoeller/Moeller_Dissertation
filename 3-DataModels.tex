...
\paragraph{Alas.}
Alas [btz] (Alas-Kluet, Batak Alas, Batak Alas-Kluet) belongs to the Malayo-Polynesian branch of the Austronesian family. It is spoken by 200,000 people on the Indonesian island of Sumatra. It is unclear if the three dialects (Alas, Kluet, and Singkil) constitute one language. The selected corpus is from the Alas dialect.  Its morphology features reduplication, infixation, and circumfixation. The corpus consists of 12 transcribed texts and one set of elicited sentences written with the Indonesian orthography. 
...
\begin{table*}
    ...
    \begin{tabular}{l|r|r|rc|rc|rc}
        ...
        % segmented in both ways with no editing: 12435/16520 = 0.752
         ...
         \hline
         Tsez & 53k & 7,315 &  & 99\% &  & 99\% & 0 & 0\% 
    \end{tabular}
    \caption[Data Statistics]{The total token count ignores multiple word expressions (when parsed as such) as single tokens. Inflected refers to the number of unique inflected forms that can be used to train the inflection task. The percentage and total number of tokens that are segmented or glossed are shown.  The segmentation and glossing task trains on both. The inflection task needs only glossed tokens.}
    \label{tab:dissdata}
\end{table*}
...
The proposed research will use supervised machine learning systems. All have achieved state-of-the-art NLP results in low-resource settings. They include the feature-based Conditional Random Fields and LSTM-based encoder-decoder, and one neural system (Transformer). All models have been trained on an NVIDIA GP102 [TITAN Xp] GPU
...
The proposed research will use the neural transformer model \citep{vaswani_attention_2017} in all three studies. Transformers have achieved promising results for NLP in low-resource languages \citep{abbott_towards_2018,Martinus2019AFO}. 
%Like most neural models, the transformer’s main drawback in low-resource settings is its sensitivity to hyper-parameters.
The proposed research will use the parameters of the Fairseq toolkit \citep{ott2019fairseq} implementation\footnote{\url{https://fairseq.readthedocs.io/en/latest/}}.
